...
PfFfMacScheduler::PfFfMacScheduler()
    : m_cschedSapUser(0), m_schedSapUser(0), m_timeWindow(99.0), m_nextRntiUl(0) {
...
PfFfMacScheduler::~PfFfMacScheduler() { NS_LOG_FUNCTION(this); }

void PfFfMacScheduler::DoDispose() {
...
TypeId PfFfMacScheduler::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::PfFfMacScheduler")
            ...
            .AddAttribute("CqiTimerThreshold", "The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
                          UintegerValue(1000), MakeUintegerAccessor(&PfFfMacScheduler::m_cqiTimersThreshold),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("HarqEnabled", "Activate/Deactivate the HARQ [by default is active].",
                          BooleanValue(true), MakeBooleanAccessor(&PfFfMacScheduler::m_harqOn),
                          MakeBooleanChecker())
            .AddAttribute("UlGrantMcs", "The MCS of the UL grant, must be [0..15] (default 0)",
                          UintegerValue(0), MakeUintegerAccessor(&PfFfMacScheduler::m_ulGrantMcs),
                          MakeUintegerChecker<uint8_t>());
    return tid;
}

void PfFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) { m_cschedSapUser = s; }

void PfFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) { m_schedSapUser = s; }

FfMacCschedSapProvider *PfFfMacScheduler::GetFfMacCschedSapProvider() { return m_cschedSapProvider; }

FfMacSchedSapProvider *PfFfMacScheduler::GetFfMacSchedSapProvider() { return m_schedSapProvider; }

void PfFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) { m_ffrSapProvider = s; }

LteFfrSapUser *PfFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }

void PfFfMacScheduler::DoCschedCellConfigReq(
...
void PfFfMacScheduler::DoCschedUeConfigReq(
...
void PfFfMacScheduler::DoCschedLcConfigReq(
...
void PfFfMacScheduler::DoCschedLcReleaseReq(
...
void PfFfMacScheduler::DoCschedUeReleaseReq(
...
void PfFfMacScheduler::DoSchedDlRlcBufferReq(
...
void PfFfMacScheduler::DoSchedDlPagingBufferReq(
...
void PfFfMacScheduler::DoSchedDlMacBufferReq(
...
int PfFfMacScheduler::GetRbgSize(int dlbandwidth) {
    for (int i = 0; i < 4; i++) {
...
unsigned int PfFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
    std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
    unsigned int lcActive = 0;
    for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
        if (((*it).first.m_rnti == rnti) &&
            (((*it).second.m_rlcTransmissionQueueSize > 0) ||
             ((*it).second.m_rlcRetransmissionQueueSize > 0) || ((*it).second.m_rlcStatusPduSize > 0))) {
            lcActive++;
        }
        if ((*it).first.m_rnti > rnti) {
            break;
...
uint8_t PfFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t PfFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
    NS_LOG_FUNCTION(this << rnti);
...
void PfFfMacScheduler::RefreshHarqProcesses() {
    NS_LOG_FUNCTION(this);
...
void PfFfMacScheduler::DoSchedDlTriggerReq(
        ...
        if (retx.at(0) || retx.at(1)) {
            ...
            for (uint16_t k = 0; k < (*itRlcPdu).second.at(0).at(dci.m_harqProcess).size(); k++) {
                ...
                for (uint8_t j = 0; j < nLayers; j++) {
                    ...
                    } else { // if no retx needed on layer j, push an RlcPduListElement_s object with m_size=0
                             // to keep the size of rlcPduListPerLc vector = 2 in case of MIMO
                        NS_LOG_INFO(" layer " << (uint16_t) j << " tb size " << dci.m_tbsSize.at(j));
                        RlcPduListElement_s emptyElement;
                        emptyElement.m_logicalChannelIdentity =
                            (*itRlcPdu).second.at(j).at(dci.m_harqProcess).at(k).m_logicalChannelIdentity;
                ...
                if ((cqi1 > 0) || (cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
                {
                    if (LcActivePerFlow((*it).first) > 0) {
                        ...
                        NS_LOG_INFO(this << " RNTI " << (*it).first << " MCS " << (uint32_t) mcs
                                         ...
                                         << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);
        ...
        for (itBufReq = m_rlcBufferReq.begin(); itBufReq != m_rlcBufferReq.end(); itBufReq++) {
            if (((*itBufReq).first.m_rnti == (*itMap).first) &&
                ...
                for (uint8_t j = 0; j < nLayer; j++) {
                    ...
                    NS_LOG_INFO(this << " LCID " << (uint32_t) newRlcEl.m_logicalChannelIdentity << " size "
                                     << newRlcEl.m_size << " layer " << (uint16_t) j);
    ...
    for (itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end(); itStats++) {
        ...
        // update average throughput (see eq. 12.3 of Sec 12.3.1.2 of LTE – The UMTS Long Term Evolution, Ed
        // Wiley)
...
void PfFfMacScheduler::DoSchedDlRachInfoReq(
...
void PfFfMacScheduler::DoSchedDlCqiInfoReq(
        ...
        if (params.m_cqiList.at(i).m_cqiType == CqiListElement_s::P10) {
            ...
            if (it == m_p10CqiRxed.end()) {
                ...
                m_p10CqiRxed.insert(std::pair<uint16_t, uint8_t>(
                    rnti, params.m_cqiList.at(i).m_wbCqi.at(0))); // only codeword 0 at this stage (SISO)
...
double PfFfMacScheduler::EstimateUlSinr(uint16_t rnti, uint16_t rb) {
...
void PfFfMacScheduler::DoSchedUlTriggerReq(
    ...
    if (m_harqOn == true) {
        ...
        for (uint16_t i = 0; i < params.m_ulInfoList.size(); i++) {
            if (params.m_ulInfoList.at(i).m_receptionStatus == UlInfoListElement_s::NotOk) {
                ...
                NS_LOG_INFO(this << " UL-HARQ retx RNTI " << rnti << " harqId " << (uint16_t) harqId << " i "
                                 << i << " size " << params.m_ulInfoList.size());
                ...
                if (free) {
                    ...
                    NS_LOG_INFO(this << " Send retx in the same RBs " << (uint16_t) dci.m_rbStart << " to "
    ...
    // Divide the remaining resources equally among the active users starting from the subsequent one served
...
void PfFfMacScheduler::DoSchedUlCqiInfoReq(
    ...
    case UlCqi_s::PUSCH: {
        ...
        NS_LOG_DEBUG(this << " Collect PUSCH CQIs of Frame no. " << (params.m_sfnSf >> 4) << " subframe no. "
                          << (0xF & params.m_sfnSf));
    ...
    case UlCqi_s::SRS: {
        NS_LOG_DEBUG(this << " Collect SRS CQIs of Frame no. " << (params.m_sfnSf >> 4) << " subframe no. "
                          << (0xF & params.m_sfnSf));
...
void PfFfMacScheduler::RefreshDlCqiMaps(void) {
    // refresh DL CQI P01 Map
    std::map<uint16_t, uint32_t>::iterator itP10 = m_p10CqiTimers.begin();
    while (itP10 != m_p10CqiTimers.end()) {
        NS_LOG_INFO(this << " P10-CQI for user " << (*itP10).first << " is " << (uint32_t)(*itP10).second
    ...
    while (itA30 != m_a30CqiTimers.end()) {
        NS_LOG_INFO(this << " A30-CQI for user " << (*itA30).first << " is " << (uint32_t)(*itA30).second
                         << " thr " << (uint32_t) m_cqiTimersThreshold);
        if ((*itA30).second == 0) {
            // delete correspondent entries
...
void PfFfMacScheduler::RefreshUlCqiMaps(void) {
    // refresh UL CQI  Map
    std::map<uint16_t, uint32_t>::iterator itUl = m_ueCqiTimers.begin();
    while (itUl != m_ueCqiTimers.end()) {
        NS_LOG_INFO(this << " UL-CQI for user " << (*itUl).first << " is " << (uint32_t)(*itUl).second
                         << " thr " << (uint32_t) m_cqiTimersThreshold);
        if ((*itUl).second == 0) {
            // delete correspondent entries
...
void PfFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid, uint16_t size) {
    std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
void PfFfMacScheduler::UpdateUlRlcBufferInfo(uint16_t rnti, uint16_t size) {
...
void PfFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti, uint8_t txMode) {
